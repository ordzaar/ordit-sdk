...
^ Note: Magic Eden Wallet only supports `mainnet`.
...
- `BrowserWalletNetworkMismatchError` if Magic Eden Wallet is not used on `mainnet`
...
- `options` : `object` (optional for Unisat)
  ...
  - `network` (for Xverse and Magic Eden Wallet only) : `"mainnet" | "testnet"` (defaults to `mainnet`)^ - Network
  - `inputsToSign` (for Xverse and Magic Eden Wallet only) : List of inputs and signing indexes to sign
