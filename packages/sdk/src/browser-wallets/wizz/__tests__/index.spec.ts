...
describe("Wizz Wallet", () => {
  ...
  // Wizz uses eth-rpc-providers which does not throw Error object
  const WIZZ_REJECT_ERROR = {
    code: 4001,
  ...
  describe("getAddresses", () => {
    test("should return address from mainnet", () => {
      ...
      vi.stubGlobal("wizz", {
        getNetwork: vi.fn().mockResolvedValue(NETWORK_TO_WIZZ_NETWORK[network]),
  ...
  describe("signPsbt", () => {
    ...
    test("should fail to sign a psbt when wizz returns an empty psbt hex string", async () => {
      const SIGN_PSBT_ERROR = new OrditSDKError(
        "Failed to sign psbt hex using wizz",
      );
      vi.stubGlobal("wizz", {
  ...
  describe("signMessage", () => {
    ...
    test("should fail to sign a message when wizz returns an empty signature", async () => {
      const SIGN_MESSAGE_ERROR = new OrditSDKError(
        "Failed to sign message using wizz",
      );
      vi.stubGlobal("wizz", {
