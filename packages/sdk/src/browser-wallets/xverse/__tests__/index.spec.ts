...
describe("Xverse Wallet", () => {
  ...
  describe("signPsbt", () => {
    ...
    test("should sign a psbt with finalize and extractTx set to true when options is not defined", async () => {
      ...
      const psbt = createMockPsbt("p2sh-p2wpkh");
    ...
    //   signTransactionSpy.mockImplementation((options: SignTransactionOptions) => {
    //     options.onCancel();
    //     return Promise.resolve();
    //   });
    //   expect(typeof window).not.toBeUndefined();
    //   expect(signPsbt("mainnet")).rejects.toThrowError();
    //   expect(signPsbt("testnet")).rejects.toThrowError();
    // });
  });
});
