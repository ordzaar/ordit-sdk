...
async function getAddresses(
  ...
  if (network !== "signet" && derivedNetwork !== network) {
    throw new BrowserWalletNetworkMismatchError(
      "Leather network mismatch, please switch it manually",
    );
  }
  // error if network is signet but derived network is not testnet (signet has same address format as testnet)
  if (network === "signet" && derivedNetwork !== "testnet") {
    throw new BrowserWalletNetworkMismatchError(
      "Leather network mismatch, please switch it manually",
    );
  }
