...
//   return ADDRESS_NAME_TO_TYPE[addressFormat as AddressFormats];
// }
...
export function getAddressesFromPublicKey(
  ...
  if (format === "all") {
    ...
    addressTypesList.forEach((addrType) => {
      if (addrType === "p2tr") {
        const paymentObj = createTransaction(
        ...
        addresses.push({
          ...
          format: ADDRESS_TYPE_TO_NAME[addrType],
    ...
    addresses.push({
      address: paymentObj.address,
      format: ADDRESS_TYPE_TO_NAME[format],
      pub: keys.publicKey.toString("hex"),
      xkey:
...
export function getAccountDataFromHdNode({
  ...
  const addressType = ADDRESS_TYPE_TO_NAME[format];
...
type GetAllAccountsFromHDNodeOptions = Omit<
  GetAccountDataFromHdNodeOptions,
  "format"
>;
